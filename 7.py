...
print("Librerías cargadas. Iniciando pipeline FINAL (Submission #17 - El Finalista)...")
...
N_PCA_COMPONENTS = 20
N_SPLITS = 5 # Lo usaremos solo para definir el split

# --- Tus parámetros ganadores (V10/V14) ---
QUANTILE_ALPHA = 0.78 
MODEL_DEPTH = 7       
MODEL_L2_REG = 5      
MODEL_LR = 0.01       
...
# --- PASO 3: ENTRENAMIENTO "FINALISTA" (V17) ---
# ===================================================
print(f"\n--- Iniciando [Paso 3]: Entrenamiento de CatBoost (V17 - Finalista) ---")
tscv = TimeSeriesSplit(n_splits=N_SPLITS)

# 1. Definir nuestro split de Hold-Out (Entrenar con el pasado, validar con el futuro)
# Tomamos los índices del ÚLTIMO split (80% train, 20% val)
train_index, val_index = list(tscv.split(X_train))[-1]

X_train_fold, X_val_fold = X_train.iloc[train_index], X_train.iloc[val_index]
y_train_fold, y_val_fold = y_train.iloc[train_index], y_train.iloc[val_index]

print(f"Set de entrenamiento (folds 1-4): {X_train_fold.shape}")
print(f"Set de validación (fold 5): {X_val_fold.shape}")

# 2. Crear Pools de CatBoost
...
# 3. Entrenar el modelo de validación (¡Solo una vez!)
model = CatBoostRegressor(
    iterations=10000,
    learning_rate=MODEL_LR,   # <-- 0.01
    depth=MODEL_DEPTH,        # <-- 7
...
print("\n--- Entrenando modelo de validación (sobre 80% de datos)... ---")
model.fit(
    train_pool,
    eval_set=val_pool
)

# 4. Evaluar en nuestro "Leaderboard Local"
val_preds = model.predict(X_val_fold)
val_preds_real = np.expm1(val_preds)
y_val_real = np.expm1(y_val_fold)

local_rmse_real = np.sqrt(mean_squared_error(y_val_real, val_preds_real))
local_r2_real = r2_score(y_val_real, val_preds_real)
best_iteration = model.get_best_iteration()

print("\n--- Evaluación del Modelo de Validación Completada ---")
print(f"Mejor iteración: {best_iteration}")
print(f"RMSE (Unidades Reales): {local_rmse_real:.4f}")
print(f"R2 (escala real): {local_r2_real:.4f}  <--- ¡COMPARA ESTE NÚMERO CON TU RÉCORD!")

# --- PASO 4: RE-ENTRENAMIENTO FINAL (¡LA MAGIA!) ---
# ==================================================
print(f"\n--- Iniciando [Paso 4]: Re-entrenamiento FINAL sobre TODOS los datos (100%) ---")

# 1. Crear un Pool con TODOS los datos de entrenamiento
...
# 2. Re-entrenar el modelo final
final_model = CatBoostRegressor(
    iterations=best_iteration,  # <-- ¡Entrenamos solo hasta la mejor iteración encontrada!
    learning_rate=MODEL_LR,
    depth=MODEL_DEPTH,
    l2_leaf_reg=MODEL_L2_REG,
    loss_function=loss_function,
    eval_metric=loss_function,
    random_seed=42,
    verbose=500
    # Sin early_stopping, queremos que use todas las iteraciones
)

final_model.fit(full_train_pool)
print("Modelo final re-entrenado.")
...
submission_filename = f'submission_catboost_V17_Finalist.csv'
submission.to_csv(submission_filename, index=False)

print("\n--- ¡PIPELINE V17 (FINALISTA) COMPLETADO! ---")
print(f"Archivo '{submission_filename}' creado con éxito.")
print(f"¡Este es el *pipeline* más robusto! Mucha suerte.")
