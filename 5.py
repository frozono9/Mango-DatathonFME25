...
print("Librerías cargadas. Iniciando pipeline FINAL (Submission #14 - Feature Normalization)...")
...
# --- PASO 2: INGENIERÍA DE CARACTERÍSTICAS (V2 - 70 features) ---
# =======================================================
...
# --- 2.9: ¡NUEVO! Normalización de Features Numéricas ---
print("Normalizando (log1p) features numéricas sesgadas...")
# Definimos las features numéricas que están sesgadas (skewed)
# Básicamente, todas nuestras features numéricas base y las agregadas
num_features_to_log = [
    'num_stores', 'num_sizes', 'price', 'life_cycle_length', 'phase_in_week'
]
# Añadimos todas las features agregadas que creamos (ej. 'price_mean_by_family')
...
for col in num_features_to_log:
    if col in df_combined.columns:
        # Aplicamos log1p(x) = log(1+x) para manejar ceros
...
# --- PASO 3: ENTRENAMIENTO DEL MODELO (FINAL - V14) ---
# ===================================================
print(f"\n--- Iniciando [Paso 3]: Entrenamiento de CatBoost (V14 - Feat Norm) ---")
...
loss_function = f'Quantile:alpha={QUANTILE_ALPHA}' # <-- Usa 0.78
...
for fold, (train_index, val_index) in enumerate(tscv.split(X_train)):
    ...
    # --- Usamos los parámetros ganadores de V10 ---
...
print(f"R2 (escala real): {oof_r2_real:.4f}  <--- ¡COMPARA ESTE NÚMERO CON TU RÉCORD!")
...
submission_filename = f'submission_catboost_V14_LogFeatures.csv'
submission.to_csv(submission_filename, index=False)

print("\n--- ¡PIPELINE V14 COMPLETADO! ---")
print(f"Archivo '{submission_filename}' creado con éxito.")
print(f"Compara tu nuevo 'R2 (escala real)' con tu récord anterior.")
