...
print("Librerías cargadas. Iniciando pipeline FINAL (Submission #2)...")
...
# --- PASO 2: INGENIERÍA DE CARACTERÍSTICAS (Versión Mejorada) ---
# =======================================================
print(f"\n--- Iniciando [Paso 2]: Ingeniería de Características ---")

# --- 2.1 a 2.6 (Carga, Agregación, Fechas, PCA, etc.) ---
# (El código es idéntico al script de prueba)
# --- 2.1: Carga de Datos ---
try:
...
# --- 2.7: Creación de Features Agregadas (¡Versión V2!) ---
print("Creando features agregadas (mean y median)...")
...
for cat_col in agg_cat_cols:
    for num_col in agg_num_cols:
        ...
        ratio_median_col = f'{num_col}_ratio_to_median_{cat_col}' # <-- NUEVA FEATURE
        ...
        df_combined[ratio_median_col] = df_combined[num_col] / (df_combined[median_col] + 1e-6) # <-- NUEVA FEATURE
print("Nuevas features de ratio (median) creadas.")
...
# ===================================================
print(f"\n--- Iniciando [Paso 3]: Entrenamiento de CatBoost (Parámetros Ganadores) ---")
...
for fold, (train_index, val_index) in enumerate(tscv.split(X_train)):
    ...
    model = CatBoostRegressor(
        ...
        random_seed=42,
        verbose=250,
        early_stopping_rounds=200 # Más paciencia
    )
...
print(f"R2 (escala real): {oof_r2_real:.4f} (¡Este es tu score local!)")
...
# --- ¡Nombra el archivo para la Submission #2! ---
submission_filename = 'submission_catboost_V2_R2_078.csv'
submission.to_csv(submission_filename, index=False)

print("\n--- ¡PIPELINE FINAL COMPLETADO! ---")
print(f"Archivo '{submission_filename}' creado con éxito.")
print("¡Mucha suerte con esta submission!")
